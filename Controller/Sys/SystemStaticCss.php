...
class SystemStaticCss extends Sys {
	...
	public function run(){
		...
		$this->loop(0, 200, 5, 'm-', '', 'margin', 'px', ' !important');
		...
		$this->loop(0, 200, 5, 'p-', '', 'padding', 'px', '!important');
		...
		$this->loop(0, 500, 5, 'MW-', '', 'max-width', 'px');
		$this->loop(0, 100, 1, 'W-', '-per', 'width', '%');
		$this->loop(0, 500, 5, 'H-', '', 'height', 'px');
		$this->loop(0, 500, 5, 'MH-', '', 'max-height', 'px');
		$this->loop(0, 100, 1, 'H-', '-per', 'width', '%');
	...
	protected function loop($from, $to, $interval, $prefix, $suffix, $property, $unit, $imp=''){
		$str = '';
		for ($i = $from; $i < $to; $i) { 

			$str .= "." . $prefix . $i . $suffix . " {";
				$str .= $property . ": " . $i . $unit . $imp . ";";
			$str .= "}" . PHP_EOL;
