...
class moduleEditIndex {
	...
	init(){
		setTimeout(f => {
			...
			this.mainHeader.pageTitle = 'Opoink Modules Edit';
			this.loader = window['_vue']['loader-component'];
			this.toast = window['_vue']['toast-component'];

			this.resetControllerForm();
			this.form.vendor_name = this.url.getQuery('vendor');
			this.form.module_name = this.url.getQuery('module');
			this.form.module_ver = this.url.getQuery('version');
			this.current_ver = this.url.getQuery('version');
		}, 500);
	}
		...
		this.request.getFormKey().then(formkey => {
			...
			.then(mod => {
				if(!mod.error && mod.result){
					this.loader.reset();
					this.url.navigateTo('/'+sysUrl+'/module/edit?vendor='+this.form.vendor_name+'&module='+this.form.module_name+'&version='+this.form.module_ver);
...
window['_vue']['module-edit-index-component'] = new moduleEditIndex();

let moduleEditIndexComponent = Vue.component('module-edit-index-component', {
	data: function(){
		return {
			vue: window['_vue']['module-edit-index-component'] 
		}
	},
	beforeRouteEnter: function(to, from, next) {
		let data = window['_vue']['module-edit-index-component'];
		data.init();
		next();
	},
	template: '{{template}}'
...
vueRouter.addRoutes([{ 
	path: '/'+sysUrl+'/module/edit', 
	component: moduleEditIndexComponent, 
	name: 'module-edit-index' 
}]);
